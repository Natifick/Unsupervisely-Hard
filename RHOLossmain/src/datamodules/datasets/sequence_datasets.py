...
class specified_indices_CIFAR10(VisionDataset):
    """The standard PyTorch class, modified so that each datapoint is returned
    as (index, sample, target) instead of (sample, target). Index is the global
    index of each datapoint w.r.t. the dataset. What follows is the docstring of
    the standard PyTorch class.
    ...
    def __init__(
        ...
        super(specified_indices_CIFAR10, self).__init__(
            root, transform=transform, target_transform=target_transform
        )
        ...
        #delete_indices = np.random.choice(10_000, size=int(0.07 * 10_000), replace=False)
        #add_indices = np.random.choice(10_000, size=int(0.07 * 10_000), replace=False)
        
        if self.delete_indices is not None:
            mask_delete = np.ones(len(self.data), dtype=bool)
            mask_delete[self.delete_indices] = False

            #mask_add = np.zeros(len(self.data), dtype=bool)
            #mask_add[add_indices] = True

            
            #self.data += self.data[mask_add]
            self.data = self.data[mask_delete]
