...
class SpecifiedCIFAR10DataModule(GoldiproxDatamodule):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dims = (3, 32, 32)
        self.num_classes = 10
        ...
        #here
        trainplusvalset_size = 50000 - len(self.delete_indices)
        train_subset = list(range(0, trainplusvalset_size, 2))
        val_subset = list(range(1, trainplusvalset_size, 2))
        ...
        if self.valset_fraction < 1:
            ...
            # all of this is only to make sure that the random subset is not very unbalanced
            temp_train_and_valset = specified_indices_CIFAR10(
                self.data_dir,
                train=True,
                transform=self.transform
                if not self.valset_data_aug
        ...
        if self.sequence is None:
            ...
            self.indices_train_factory = lambda: specified_indices_CIFAR10(
                self.data_dir,
                train=True,
                transform=self.transform,
                sequence=self.sequence,
        ...
        self.indices_test_factory = lambda: specified_indices_CIFAR10(
            self.data_dir, train=False, transform=self.transform
        )
        if self.trainset_data_aug:
            log.info("Trainset data augmentation turned on")
